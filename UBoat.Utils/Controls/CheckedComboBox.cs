...
/*
 * Sourced from CodeProject: http://www.codeproject.com/Articles/31105/A-ComboBox-with-a-CheckedListBox-as-a-Dropdown
 */

namespace UBoat.Utils.Controls
{
    public class CheckedComboBoxEx : CheckedComboBox
    {
        public class SelectAllListItem
        {
        ...
        private bool m_ShowSelectAll;

        public bool ShowSelectAll
        {
            get
            {
                return m_ShowSelectAll;
            }
            set
            {
                m_ShowSelectAll = value;
                zUpdateShowSelectAll();
            }
        }

        public CheckedComboBoxEx() 
            : base()
        {
            this.ItemCheck += CheckedComboBoxEx_ItemCheck;
        }

        void CheckedComboBoxEx_ItemCheck(object sender, ItemCheckEventArgs e)
        {
            if (m_ShowSelectAll)
            {
                this.ItemCheck -= CheckedComboBoxEx_ItemCheck;
                if (e.Index == 0)
                {
                    if (e.NewValue == CheckState.Checked)
                    {
                        ...
                        {
                            for (int x = 1; x < this.Items.Count; x++)
                            {
                                this.SetItemChecked(x, true);
                            }
                            this.SkipFirstCheckedItemForString = true;
                        }
                        else
                        {
                    ...
                    else
                    {
                        if (this.Items.Count > 1)
                        {
                            for (int x = 1; x < this.Items.Count; x++)
                            {
                                this.SetItemChecked(x, false);
                            }
                        }
                        this.SkipFirstCheckedItemForString = false;
                    }
                }
                else
                {
                    if (e.NewValue == CheckState.Checked && this.CheckedIndices.Count == this.Items.Count - 2)
                    {
        ...
        private void zUpdateShowSelectAll()
        {
            if (m_ShowSelectAll)
            {
                if (this.Items.Count == 0 || !(this.Items[0] is SelectAllListItem))
                {
                    this.Items.Insert(0, new SelectAllListItem());
                }
            }
            else
            {
    ...
    public class CheckedComboBox : ComboBox {
        ...
        internal class Dropdown : Form {
            ...
            public bool SkipFirstCheckedItemForString { get; set; }

            public string GetCheckedItemsStringValue() {
                int startIndex = SkipFirstCheckedItemForString ? 1 : 0;
                StringBuilder sb = new StringBuilder("");
                for (int i = startIndex; i < cclb.CheckedItems.Count; i++) {                    
        ...
        protected bool SkipFirstCheckedItemForString
        {
            get
            {
                return dropdown.SkipFirstCheckedItemForString;
            }
            set
            {
                dropdown.SkipFirstCheckedItemForString = value;
            }
        }
